...
| Insert equation | "equation", "formula", "math" | "公式", "插入公式", "数学表达式" | `word-agent:word-edit` |
| Cross-reference | "cross-reference", "ref to figure" | "交叉引用", "引用图/表" | `word-agent:word-edit` |
| Watermark | "watermark", "draft mark" | "水印", "添加水印" | `word-agent:word-submit` |
| Undo edit | "undo", "revert last change" | "撤销", "回退", "撤回上一步" | `word-agent:word-edit` |
| Bulk edit | "bulk edit", "batch changes" | "批量修改", "批量替换" | `word-agent:word-edit` |
| Structural check | "structural validation", "paraId", "bookmarks" | "结构检查", "文档结构", "书签" | `word-agent:word-check` |
| Layout diagnostics | "layout check", "page overflow" | "布局检查", "排版诊断", "页面溢出" | `word-agent:word-check` |
...
1. **Document Map First**: Most modules need a Document Map. The map is **persisted as a file** at `{文档所在目录}/.word-agent/{文档名去扩展名}.map.md`（由 word-read 写入，不是仅通过对话上下文传递）。下游模块优先 Read 该文件；有效性由 mtime 判定（map 文件 mtime 晚于 .docx 的 mtime 才有效，写操作后 docx 变新即自动过期）。If no valid map file exists, orchestrator runs word-read first automatically.
...
7. **Text Box Awareness**: `get_document_text` and `docx2python` both completely skip text box content (`w:txbxContent`), and text boxes often live outside document.xml (university templates put format annotations in header text boxes, e.g. `word/header2.xml`). word-reader and word-formatter MUST run `python3 scripts/extract_textboxes.py "{file_path}"` — it scans ALL XML parts and dedupes mc:Fallback copies. See `references/tool_routing.md` and `skills/word-read/SKILL.md` Step 3.5.

8. **MANDATORY Font Normalization Gate**: After ANY operation that modifies a Word document (format, edit, table/figure, reference, review), you MUST run `python3 scripts/normalize_fonts.py "{file_path}" --unify` as the final step before returning the document to the user. This is non-negotiable — MCP tools produce broken font attributes (missing eastAsia, theme references overriding explicit fonts, bare runs with no font spec). The script fixes all three layers (styles.xml, existing rFonts, bare runs). Do NOT skip this step. Do NOT treat it as optional. See `references/font_normalization.md`.
