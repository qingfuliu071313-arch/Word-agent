...
7. **Text Box Awareness**: `get_document_text` and `docx2python` both completely skip text box content (`w:txbxContent`). Format templates frequently use text boxes for formatting annotations. word-reader MUST extract text boxes via XML parsing (zipfile + ElementTree). See `references/tool_routing.md` and `skills/word-read/SKILL.md` Step 3.5.
